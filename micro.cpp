...
int fd_socket = -1;
usi_read_cb *usi_read;
usi_write_cb *usi_write;
...
int socket_recv_line(string &s) {
    // 末尾の改行コードは含まない
    s.clear();
    char buf[4096];
    int size = usi_read(buf);
    s += buf;
    return size;
}
...
extern "C" int micro_main(usi_read_cb _usi_read, usi_write_cb _usi_write) {
    usi_read = _usi_read;
    usi_write = _usi_write;
